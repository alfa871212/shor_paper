...
def myAQFT(n,appro,inverse=False):
    qr = QuantumRegister(n)
    gate=QFT(n,inverse=inverse,do_swaps=False,approximation_degree=appro).to_gate()
    gate.name='QFT'
    return gate
...
def adder_latex(a,b,n):
    ...
    circuit_drawer(qc,output='latex_source',filename='./latex/test.tex',plot_barriers=False)
def gate_add(n,num):
    qc =QuantumCircuit(n)
    ret = qc.to_gate()
...
def ccphiADDmodN(n, a, b, N, print_qc = False, save_fig=False):
    ...
    if save_fig:
        ...
        circuit_drawer(qc,scale=1.4,filename=filename,output='mpl',style={'fontsize':20},plot_barriers=False)
...
def cmult_a_mod_N(n, a, b,N, print_qc=False,save_fig=False):
    ...
    if print_qc:
        print(qc)
    if save_fig:
        circuit_drawer(qc,scale=0.7,output='mpl',filename='./report/cmult.png')
...
def cu_a(n, a, N,print_qc=False,save_fig = False):
    ...
    if print_qc:
        print(qc)
    if save_fig:
        circuit_drawer(qc,output='mpl',scale=0.8,filename='./report/cua.png')
    gate = qc.to_gate()
...
def shor_normal_latex(n):
    ...
    qc.x(down[n-1])
    for i in range(2*n):
        qc.append(shor_gate_latex(2*n+2,i).control(),qargs=[up[i]]+down[:]+down_b[:]+ancilla[:])
...
def shor_seq_latex(n):
    ...
    for i in range(2*n):
        ...
        qc.append(shor_gate_latex(2*n+2,i).control(),qargs=ctrl[:]+down[:]+down_b[:]+ancilla[:])
        qc.h(ctrl)
        #qc.measure(ctrl,i)
