...
def test_adder_appro(a,b,n,appro,args):
    ...
    print("="*40)
    print(f"Executing adder_appro with a={a}, b={b}, n={n}, appro_deg={appro}...")
    if args.draw:
...
def test_adder(a,b,n,args):
    ...
    if args.output:
        ...
        plot_histogram(res,figsize=(10,10),title=f'adder{a}_{b}').savefig(path)
...
def test_ccphiMOD(n,b,a,N,args,print_qc=False,save_fig=False):
    ...
    gate=ccphiADDmodN(n=n,a=a,b=b,N=N,print_qc=print_qc,save_fig=save_fig)
    ...
    if print_qc:
        print(qc)
    if save_fig:
        circuit_drawer(qc,scale=1.3,output='mpl',filename='./report/ccphiaddmod.png',plot_barriers=False)
    res = sim.mySim(qc,args)
    if len(list(res))!=1:
...
def rangeTest_ccphiMOD(N) :
    ...
    for i in range(1,16):
        for j in range(1,16):
            try:
                test_ccphiMOD(n=4,b=i,a=j,N=N,print_qc=False,save_fig=False)
            except Exception as e:
                print(e)
                args = (5,i,j,N)
                if str(e)=='wrong ans':
...
def shorNormal(N,a,args):
    ...
    if args.draw:
        ...
        if not os.path.exists(qcpath):
            os.makedirs(qcpath)
        circuit_drawer(qc,output='mpl',filename=f'./normal/circuit/{N}_{a}.png',scale=0.6)
    ...
    if args.output:
        ...
        if not os.path.exists(respath):
            os.makedirs(respath)
        plot_histogram(res,figsize=(10,10),title=f'N={N} a={a} result(Nor)').savefig(respath+f'/{N}_{a}_res.png')
...
def shorSequential(N,a,args):
   ...
   # print(new_dict)
    ...
    if args.output:
        ...
        if not os.path.exists(respath):
            os.makedirs(respath)
        plot_histogram(new_dict,figsize=(10,10),title=f'N={N} a={a} result(Seq)').savefig(respath+f'/{N}_{a}_res.png')
